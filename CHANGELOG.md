...
### TUI
- 牌局结束后新增“下一局”按钮与 `N` 快捷键，可按头游所在队伍最终级牌重新发牌，避免结束态卡住。
- 修复下一局承接规则：下一局以头游所在队伍的最终级牌继续发牌；三游和末游不同队时由末游先手，同队双贡时由进贡牌大者先手。
- 打牌界面手牌改为显式底色牌面，并加入中文花色、花色符号、光标、选中和逢人配标记，降低终端主题对可读性的影响。
- 修复真人末游时历史统计保存无法取得名次的问题。

### 规则
- `GameState` 现在记录两队各自的开局级牌，结算时只按头游方本队级牌升级，另一队保持自己的级牌，避免连续多局时把两队都错误套用桌面级牌。
- 新增下一局先手判定 helper，按三游/末游队伍关系和新局最大进贡牌比较决定起牌者。
...
- MCTS 候选剪枝强制保留合法一手出完动作，并优先展开，避免 Top-N 截断漏掉终局牌。
- 戴长胜档对手短手牌时的必用炸弹不再被配合意识覆盖，提升残局拦截稳定性。

...
> 状态：**M5.1 优化中**（持久化主体完成，断点续局恢复正在补齐）
> 规则：与全国锦标赛通用口径一致
> AI：5 档（新手 / 进阶 / 高手 / 职业（IS-MCTS）/ **戴长胜（初步风格化）**）
> 持久化：Profile / Savegame / History 已实现，续局恢复为 M5.1 重点

## 特性

- 🎴 严格按全国掼蛋比赛规则：2 副牌 108 张、4 人固定搭档、级牌、进贡/还贡、报牌、漂牌、逢人配、抗贡、过 A
- 🤖 5 档 AI 难度，最高档致敬戴长胜牌风（IS-MCTS + 炸弹吝啬 + 配合意识，控场/漂牌仍在调优）
- 🔁 历史战绩 / 自动保存 / 断点续局恢复
...
- [x] **M4** 戴长胜 AI（v0.5.0）
- [~] **M5** 持久化（v0.6.0）：保存/历史/统计完成，续局恢复补齐中
- [ ] **M5.1** 断点续局恢复与项目状态校准（v0.6.1）
- [ ] M6 调优 & v1.0 发布
